...
elements.refreshDevicesBtn.addEventListener('click', () => {
  ...
  loadAudioDevices().finally(() => setTimeout(() => elements.refreshDevicesBtn.classList.remove('spinning'), 500));
});
...
// 2. Setup Recording Controls
elements.recordBtn.addEventListener('click', () => {
  if (isRecordingState()) {
    stopRecording();
  } else {
    // Pass the callback: What happens when recording finishes?
