...
export async function startRecording(onComplete: OnRecordingComplete): Promise<void> {
  ...
  try {
    ...
    if (systemAudioEnabled) {
      try {
        ...
        if (screenSource) {
           const systemStream = await navigator.mediaDevices.getUserMedia({
             ...
             video: {
               // @ts-expect-error - Electron constraint
               mandatory: { chromeMediaSource: 'desktop', chromeMediaSourceId: screenSource.id, maxWidth: 1, maxHeight: 1, maxFrameRate: 1 }
             }
           });
      ...
      } catch (err) { console.warn('System audio error', err); }
    }

    for (const deviceId of activeDeviceIds) {
      try {
        const stream = await navigator.mediaDevices.getUserMedia({
          audio: { deviceId: { exact: deviceId }, echoCancellation: true, noiseSuppression: true, sampleRate: 44100 }
        });
    ...
    mediaRecorder.onstart = () => { recordingStartTime = new Date(); };
...
async function processRecording(onComplete: OnRecordingComplete) {
  ...
  const result = await window.electronAPI.saveRecording({ buffer: arrayBuffer, filename });
  ...
  setTimeout(() => { elements.statusText.textContent = 'Ready to record'; }, 2000);
}
