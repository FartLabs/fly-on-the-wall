...
export function getActiveInputDeviceIds(): string[] {
  const allDeviceItems = Array.from(elements.devicesList.querySelectorAll('.device-item'));
...
export async function loadAudioDevices(): Promise<void> {
  elements.devicesList.innerHTML = '<p class="loading-text">Loading microphones...</p>';
  ...
  try {
    ...
    if (audioDevices.length === 0) {
      elements.devicesList.innerHTML = '<p class="no-devices">No microphones found</p>';
      return;
    }
