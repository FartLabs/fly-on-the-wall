...
export async function runTranscription(buffer: ArrayBuffer, timestamp: string): Promise<void> {
  ...
  try {
    const result = await transcribeAudio(buffer, { modelSize, onProgress: updateProgress });
...
export function setupTranscriptionListeners() {
  elements.copyTranscriptionBtn.addEventListener('click', async () => {
    ...
    setTimeout(() => elements.copyTranscriptionBtn.textContent = originalText, 2000);
  });
  ...
  elements.saveTranscriptionBtn.addEventListener('click', async () => {
    ...
    const result = await window.electronAPI.saveTranscription({ text: lastTranscription, filename });
    if (result.success) {
      elements.saveTranscriptionBtn.textContent = '✓ Saved!';
      setTimeout(() => elements.saveTranscriptionBtn.textContent = '💾 Save', 2000);
    }
  });
}